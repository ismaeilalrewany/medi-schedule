...
const AppointmentsPage = ({ endpoint, isViewerAdmin = false, role }) => {
  ...
  const availableTime = useCallback(
    (selectedDoctor, selectedDate) => {
      if (selectedDoctor && selectedDate && doctors.length > 0) {
        ...
        if (doctor && doctor.availableTimeSlots) {
          const slot = doctor.availableTimeSlots.find(
            slot => new Date(selectedDate).getDay() === getDayIndex(slot.day) && slot.isAvailable
          )
          if (slot) {
  ...
  return (
    <>
          ...
          <section className="bg-white rounded-lg p-4 mb-6 ring-1 ring-neutral-300 shadow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="self-end">
                <button type="button" className="btn text-neutral/80" onClick={handleSubmitFilters}>
                  Apply Filters
                </button>
              </div>
            </div>
                ...
                {appointments.map(appointment => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-sm font-semibold capitalize ${getStatusColor(appointment.status)}`}
                      >
                        {appointment.status}
                      </span>
                    </div>
                    ...
                    <div className="flex space-x-2 pt-4 border-t border-neutral-200">
                      {(appointment.status === 'pending' || appointment.status === 'confirmed') && (
                        <button className="btn text-neutral btn-sm flex-1">Cancel</button>
                      )}
                      <button
      ...
      <Modal
        ...
        onSubmit={handleBookModalSubmit}
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                {doctors.length > 0 ? (
                  doctors.map(doctor => (
                    <option className="capitalize" key={doctor.id} value={doctor.id}>
            ...
            <div>
              ...
              <div className="flex gap-4">
                <input
                  ...
                  name="startTime"
                  className="input input-bordered w-full"
                  disabled={!availableTime(selectedDoctor, selectedDate).startTime}
                  value={startTime}
                  onChange={e => setStartTime(e.target.value)}
                  min={availableTime(selectedDoctor, selectedDate).startTime}
                  max={availableTime(selectedDoctor, selectedDate).endTime}
                  required
                />
                <input
                  type="time"
                  name="endTime"
                  className="input input-bordered w-full"
                  disabled={!availableTime(selectedDoctor, selectedDate).endTime}
                  value={endTime}
      ...
      <Modal
        ...
        showSubmitButton={role === 'doctor'}
      >
        {selectedAppointment && (
          <div className="space-y-3">
            <p className="capitalize">
              <strong>Doctor:</strong> {selectedAppointment.doctor.fullName} (
              {selectedAppointment.doctor.specialization}) {role === 'doctor' && '(You)'}
            </p>
            <p className="capitalize">
              <strong>Patient:</strong> {selectedAppointment.patient.fullName}{' '}
              {role === 'patient' && '(You)'}
            </p>
            <p>
              <strong>Date:</strong> {new Date(selectedAppointment.date).toDateString()}
            </p>
            <p>
            ...
            <p>
              ...
              {role === 'doctor' ? (
                ...
                <span
                  className={`ml-2 px-2 py-1 rounded-full text-xs font-semibold capitalize ${getStatusColor(selectedAppointment.status)}`}
                >
                  {selectedAppointment.status}
                </span>
              )}
            ...
            <p className="capitalize">
              <strong>Booked By:</strong> {selectedAppointment.createdBy}
            </p>
          </div>
        )}
      </Modal>
