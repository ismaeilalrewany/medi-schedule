...
const AvailableTimeSlotsEditor = ({
  initialSlots = [],
  onSave,
  onCancel
}) => {
  ...
  const handleSaveSlot = () => {
    if (slotRemoved) {
      ...
      const updated = tempSlots.map((slot, i) =>
        i === editingSlotIndex ? editingSlot : slot
      )
      setTempSlots(updated)
    }
      ...
      {editingSlotIndex === null ? (
        <>
          <div className="py-4">
            ...
            <button type="button" className="btn btn-outline btn-sm mb-4 w-full" onClick={handleAddSlot}>Add Time Slot</button>
            <div className="available-time-items-wrapper border border-neutral/20 rounded-md overflow-hidden bg-base-100">
              {tempSlots && tempSlots.length > 0 ? tempSlots.map((slot, idx) => (
                <div
                  key={slot._id || idx}
          ...
          <div className="modal-action mt-4 pt-4 border-t border-neutral/20">
            <button className="btn btn-ghost text-neutral/80" onClick={onCancel}>Cancel</button>
            <button className="btn bg-neutral text-neutral-content border-0" onClick={handleSaveAllSlots}>Save Changes</button>
          </div>
        </>
      ) : (
        <>
          {slotRemoved ? (
            <>
              <div className="py-4 text-center text-neutral">
                This time slot has been removed.
              </div>
              <div className="modal-action mt-4 pt-4 border-t border-neutral/20 flex justify-end gap-2">
                <button className="btn btn-ghost text-neutral/80" onClick={handleCancelSlot}>Cancel</button>
                <button className="btn bg-neutral text-neutral-content border-0" onClick={handleSaveSlot}>Save Changes</button>
              </div>
            </>
          ) : editingSlot ? (
          <>
